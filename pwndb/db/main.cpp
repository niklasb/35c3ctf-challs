...
void parser_tests() {
  ...
  }
  {
    ...
    //cout << "rest=" << res.rest.str << endl;
    //cout << (void*)inp.c_str() << " " << (void*)res.rest.str << endl;
    //cout << "rest=" << res.rest.str << endl;
  }
  {
  ...
  while (getline(cin, query)) {
    {
      ...
      if (res.valid) {
        ...
        if (logging) {
          ...
          AllCursor* c = dynamic_cast<AllCursor*>(cursor.get());
          if (c)
            log << "AllCursor @ " << c->m_cur << " " << c->m_end << endl;
        }
#endif
        if (!cursor) {
...
#else
        auto cursor = db.select(res.value);
        cout << "START RESULTS " << res.value.table_name << endl;
        while (cursor->valid()) {
          cout << "  ";
          bool fst=1;
          for (auto x : *cursor->value()) {
            if (!fst) cout << ",\t";
            cout << x;
            fst=0;
          }
...
#if LOGGING
        if (logging) {
          auto& recs = db.m_tables[res.value.table_name]->m_records;
          auto& idx = db.m_tables[res.value.table_name]->m_indexes[0].entries;
          log << "new capacity=" << recs.capacity() << "/" << recs.capacity()* sizeof(*recs.data()) << " @ " << recs.data() << endl;
          log << "new index cap=" << idx.capacity() << "/" << idx.capacity() * sizeof(*idx.data()) << " @ " << idx.data() << endl;
        }
#endif
        continue;
      }
    }
    {
      ...
      if (res.valid) {
        ...
        if (logging) {
          auto* table = db.m_tables[res.value.table_name].get();
          log << "table allocated at " << table << " (size=" << sizeof(*table) << endl;
        }
    ...
    if (query == "startlog;") {
      logging = 1;
      cout << "ok logging started;\n" << flush;
      continue;
    }

    if (query == "stoplog;") {
      logging = 0;
      cout << "ok logging stopped;\n" << flush;
      continue;
    }
