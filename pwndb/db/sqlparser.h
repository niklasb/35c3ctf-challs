...
Result<Condition> term(Parser p) {
  using namespace std;
  //cout << "term(" << p.str << ")"<<endl;
  //cout << "FOO rest=" << p.str << endl;
  auto res = middle(
      ...
      p);
  if (res.valid) {
    //cout << "YES rest=" << res.rest.str << endl;
    return res;
  }
  //cout << "NO rest=" << p.str << endl;

  SKIP(Condition, maybe_whitespace);
  SUB(Condition, column, identifier);
  //cout << "C column=" << column << endl;
  SKIP(Condition, maybe_whitespace);
...
Result<Condition> and_term(Parser p) {
  using namespace std;
  //cout << "and_term(" << p.str << ")"<<endl;
  SUB(Condition, left, term);
  //cout << "GOT LEFT" << endl;
  //cout << "rest=" <<p.str << endl;
  ...
  auto res = extra(p);
  if (res.valid) {
    //cout << "GOT AND with rest " << res.rest.str << endl;
...
Result<Condition> or_term(Parser p) {
  ...
  //cout << "(OR)GOT LEFT" << endl;
  //cout << "(OR)rest=" <<p.str << endl;

  auto extra = [](Parser p) {
    SKIP(Condition, whitespace);
    SKIP(Condition, BINDP(exact, "or"));
    //cout << "consumed or, rest=" << p.str << endl;
    return or_term(p);
  };
